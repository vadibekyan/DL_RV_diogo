...
def _worker_chunk(
    *,
    ...
    medium: str,
    mask_medium: str,
    vrad_grid: tuple[float, float, float],
    exclude_tellurics: bool,
    ccf_err_scale: bool,
    target: str,
) -> str:
    chunk_dir_path = Path(chunk_dir)
    spec_dir = chunk_dir_path / f"shifted_chunk_{chunk_id:04d}"
    spec_dir.mkdir(parents=True, exist_ok=True)

    files: list[str] = []
    for i, rv in enumerate(rv_chunk):
        wave_shifted, flux_out, flux_err_out = apply_rv_shift_on_original_grid(
            wave, flux, float(rv), flux_err=flux_err
        )
        out_csv = spec_dir / f"spectrum_{i:06d}.csv"
        pd.DataFrame(
            {
    ...
    obj.data.get_aux_data(mask_path=mask_path, mask_medium=mask_medium)
    ...
    df_chunk = pd.DataFrame(ccf_val, columns=ccf_cols)
    df_chunk.insert(0, "rv_pred", vrad_val)
    df_chunk.insert(1, "rv_err", vrad_err)
    df_chunk.insert(2, "rv_true", rv_chunk.astype(float))

    chunk_csv = chunk_dir_path / f"dataset_chunk_{chunk_id:04d}.csv"
    df_chunk.to_csv(chunk_csv, index=False)
    return str(chunk_csv)


def generate_random_rv_ccf_dataset_parrallel(
    ...
    *,
    ...
    rv_chunks = np.array_split(rv_true, n_chunks)

    chunk_csv_paths: list[str] = []
    with tempfile.TemporaryDirectory(prefix="arve_parallel_") as tmpdir:
        futures = []
        with ProcessPoolExecutor(max_workers=n_chunks) as ex:
            ...
            with tqdm(total=len(futures), desc="Random RV + ARVE CCF", unit="chunk") as pbar:
                for fut in as_completed(futures):
                    chunk_csv_paths.append(fut.result())
                    pbar.update(1)

        dfs = [pd.read_csv(p) for p in sorted(chunk_csv_paths)]
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Parallel random RV-shifted dataset generation and ARVE CCF extraction "
            "(one sample per row)."
        )
    )
...
def main() -> None:
    ...
    df = generate_random_rv_ccf_dataset_parrallel(
