#!/usr/bin/env python3
"""
Run a learning-curve experiment for RV prediction on a processed dataset using a plain 1D CNN.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a learning-curve experiment for RV regression on a CSV or parquet dataset using a plain 1D CNN."
    )
    parser.add_argument(
        "--input-path",
        default=None,
...
def build_model(
    ...
    model = keras.Model(inputs=inputs, outputs=outputs, name="plain_conv1d_rv_regressor")
    model.compile(
