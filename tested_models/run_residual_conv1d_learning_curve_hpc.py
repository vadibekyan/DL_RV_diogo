#!/usr/bin/env python3
"""
Run a learning-curve experiment for RV prediction on a processed dataset using a residual 1D CNN.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a learning-curve experiment for RV regression on a CSV or parquet dataset using a residual 1D CNN."
    )
    parser.add_argument(
        "--input-path",
        default=None,
...
def residual_block(
    ...
    y = layers.Activation("relu", name=f"{block_name}_relu_out")(y)

    if dropout > 0:
        y = layers.Dropout(dropout, name=f"{block_name}_dropout")(y)
...
def build_model(
    ...
    if len(kernel_sizes) == 1:
        kernel_sizes = kernel_sizes * len(conv_filters)
    elif len(kernel_sizes) != len(conv_filters):
        raise ValueError("kernel_sizes must have length 1 or match conv_filters length.")
    ...
    for i, (filters, kernel_size) in enumerate(zip(conv_filters, kernel_sizes), start=1):
        x = residual_block(
            x=x,
            filters=filters,
            kernel_size=kernel_size,
            block_name=f"resblock_{i}",
            dropout=dropout,
            reg=reg,
        )
    ...
    model = keras.Model(inputs=inputs, outputs=outputs, name="residual_conv1d_rv_regressor")
    model.compile(
