#!/usr/bin/env python3
"""
Predict RV values for real CCFs using a saved MLP model bundle.

Supported inputs:
1. CSV/parquet files with columns matching the saved ccf column names.
2. NPY files containing shape (n_features,) or (n_samples, n_features).

Example:
    python predict_real_ccf_mlp.py \
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Predict RVs for real CCFs using a saved MLP model.")
    parser.add_argument("--model-dir", required=True, help="Directory containing model and preprocessing files.")
    ...
    parser.add_argument("--id-col", default=None, help="Optional identifier column to preserve in CSV/parquet inputs.")
...
def main() -> None:
    ...
    y_pred_scaled = model.predict(X_scaled, verbose=0).reshape(-1)
